	...
	<ul class="nav pull-right">
		...
		<li class="dropdown">
			<a href="#" class="dropdown-toggle" data-toggle="dropdown" style="padding: 3px;line-height:32px"><?php echo (session('name')); ?>&nbsp;
				<?php if((F('img_id'))): ?><img src="<?php echo F('img_id');?>" class="avatar" onerror="this.src='__PUBLIC__/img/avatar_default.png'"/>
				<?php else: ?>
					<img src="__PUBLIC__/img/avatar_default.png" class="avatar" /><?php endif; ?>
            </a>
			<ul class="dropdown-menu">
	...
	<div class="nav_menu_tool_tips" close-status="open">
		...
		<div class="tips_panel">
			...
			<div class="tips_item" id="task_item">
				<span class="tips_count">0</span> 个新工单提醒<span class="tips_link"><a href="<?php echo U('task/index');?>">查看工单</a></span>
			</div>
...
function message_tips(){
	$.get("<?php echo U('message/tips');?>", function(data){
		...
		if(data.data['task_count'] != '0'){
			$('#task_item').css({display:'block'});	//显示工单卡片
		}else{
			$('#task_item').css({display:'none'});	//隐藏工单卡片
		}
		...
		//根据站内信、工单、日程、合同是否存在数据来判断是否显示卡片提示
...
$(function(){
	...
	/** 点击卡片提醒关闭按钮，永久关闭工单、日程、合同，暂时关闭站内信 */
