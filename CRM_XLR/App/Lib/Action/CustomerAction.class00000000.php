...
//	模糊判断
//能否添加判断
	public function check_1($name){
		import("@.ORG.SplitWord");
		$sp = new SplitWord();
		$m_customer = M('customer');
		$sql = "name like '%".$name."%'";
		$status = $m_customer->where($sql)->select();
	...
	public function validate() {
		if($this->isAjax()){
			...
			if($this->_request('clientid','trim')) {
				//判断是否有能模糊查询到的记录  yb
				$name = $_GET['name'];
				$cz_status = $this->check_1($name);
//				var_dump($cz_status);die();
				$where['is_deleted'][] .= 'eq';
				$where['is_deleted'][] .= '0';
				if ($m_customer->where($where)->find() || $cz_status==1) {  //还需要或一个  模糊查询到结果的判断  yb
					$this->ajaxReturn("","",1);
				} else {
					$this->ajaxReturn("","",0);
				}
...
//			var_dump($_POST);die();
			...
			$arrCuttomer = $objCustomer->where("name = '".$_POST['name']."'")->select();
			$objCustomer->where('customer_id='.$arrCuttomer[0]['customer_id'])->delete();
			...
			$list = field_list_html("add","customer",$leads);
//			echo "<pre>";
//			$list["main"][3]['html'] = "<br/>". $list["main"][3]['html'];
//			var_dump( $list["main"][3]['html']);die();
            $this->field_list = $list;
			$this->refer_url=$_SERVER['HTTP_REFERER'];
//						echo "<pre>";print_r($this->field_list[main][1]);die();
            $alert = parseAlert();
            $this->alert = $alert;
...
//		}
		...
		if(trim($_GET['act'] == 'sms')){
			...
			$p = isset($_GET['p']) ? intval($_GET['p']) : 1 ;
			//15
			$list = $d_v_customer->where($where)->order($order)->page($p.',50')->select();

//			echo "<pre>";
//			echo $d_v_customer->getLastSql();
//			$mx = M('Customer');
//			var_dump($d_v_customer->select());
//			die();
			$count = $d_v_customer->where($where)->count();
			...
			if($by == 'deleted') {
				...
				foreach ($list as $k => $v) {
					...
					$list[$k]["owner"] = D('RoleView')->where('role.role_id = %d', $v['owner_role_id'])->find();
					$list[$k]["creator"] = D('RoleView')->where('role.role_id = %d', $v['creator_role_id'])->find();
					$list[$k]["contact"] = D('contacts')->where('contacts_id='.$v['contacts_id'])->find();
					$days =  M('Customer')->where('customer_id = %d', $v['customer_id'])->getField('update_time');
					$list[$k]["days"] = $outdays-floor((time()-$days)/86400);
				}
			}
...
//			print_r($list);die();
			...
			$this->field_array = getIndexFields('customer');
//			echo "<pre>";
//			print_r(getMainFields('customer'));die();
			$this->field_list = getMainFields('customer');
			$this->alert = parseAlert();
			$this->display();
		}
...
//		echo "<pre>";
		$this->customer_seo = $objCustomerSeo->where('customer_id='.$_GET['id'])->select();
	...
	public function excelExport($customerList=false){
		...
		$contacts_fields_list[0]['name'] = '联系人';
		$contacts_fields_list[1]['field'] = 'telephone';
		$contacts_fields_list[1]['name'] = '座机';
		$contacts_fields_list[2]['field'] = 'email';
		$contacts_fields_list[2]['name'] = '邮件';
		$contacts_fields_list[3]['field'] = 'qq';
		$contacts_fields_list[3]['name'] = 'qq';
		$contacts_fields_list[4]['field'] = 'address';
		$contacts_fields_list[4]['name'] = '联系地址';
		$contacts_fields_list[5]['field'] = 'description';
		$contacts_fields_list[5]['name'] = '备注';
	...
	**/
	...
	public function excelImport_rc(){
	...
	**/
    public function excelImportDownload(){
		...
		$contacts_fields_list[2]['name'] = '邮件';
		$contacts_fields_list[3]['name'] = 'QQ';
		$contacts_fields_list[4]['name'] = '地址(长文本)';
		$contacts_fields_list[5]['name'] = '备注';
...
/**
	...
	*
	**/
    public function excelImportDownload_rc(){
		C('OUTPUT_ENCODE', false);
        import("ORG.PHPExcel.PHPExcel");
		$objPHPExcel = new PHPExcel();
		$objProps = $objPHPExcel->getProperties();
		$objProps->setCreator("5kcrm");
		$objProps->setLastModifiedBy("5kcrm");
		$objProps->setTitle("5kcrm contacts");
		$objProps->setSubject("5kcrm contacts Data");
		$objProps->setDescription("5kcrm contacts Data");
		$objProps->setKeywords("5kcrm contacts Data");
		$objProps->setCategory("5kcrm");
		$objPHPExcel->setActiveSheetIndex(0);
		$objActSheet = $objPHPExcel->getActiveSheet();
