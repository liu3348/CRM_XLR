...
class UserAction extends Action {
	...
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('logout','role_ajax_add','getrolebydepartment','dialoginfo','edit','seo_add', 'listdialog', 'mutilistdialog', 'getrolelist', 'getpositionlist',  'weixin','changecontent')
		);
...
//		$this->customer_id = $_GET['id'];
//		var_dump($this->customer_id);die();
		if(isset($_POST['customer_id'])){
			$objCustomerSeo = M('CustomerSeo');
			$data = array();
			$data['customer_id'] = $_POST['customer_id'];
			$data['world'] = $_POST['world'];
			$data['url'] = $_POST['url'];
			$data['contract_days'] = $_POST['contract_days'];
			$data['univalent_days'] = $_POST['univalent_days'];
			$data['status'] = $_POST['status'];
			$data['start_time'] = date('Y-m-d H:i:s');
			$data['seo_search'] = 'baidu';
			$data['largess_days'] = $_POST['largess_days'];
			if($objCustomerSeo->add($data)){
				alert('success', '添加成功', U('customer/view').'&id='.$data['customer_id']);
			}else{
				alert('error', '添加失败', U('customer/view').'&id='.$data['customer_id']);
			}
		}
		$this->display();
	}
...
alert('error', L('CAN_NOT_DELETE_USER'), U('user/index'));
	...
	public function edit(){
		...
		$m_role = M('Role');
			//修改辅助权限start (yb)
			if(isset($_POST['position_id_1'])){
				$data['assist_id'] = $_POST['position_id_1'];
				$m_role->where('user_id='.$_POST['user_id'])->save($data);
			}
//		echo "<pre>";
//		var_dump($m_role->getLastSql());die();
			//修改辅助权限end
//		获取辅助权限显示id
		//权限子级
		$arrRole = $m_role->where('role_id='.$_GET['id'])->select();
		$arrRole = $arrRole[0];
		$this->assign('arrRole', $arrRole);

		//权限父级
		$modelPosition = M('Position');
		$arrPosition = $modelPosition->where('position_id='.$arrRole['assist_id'])->select();
		$arrPosition = $arrPosition[0];
		$this->assign('arrPosition', $arrPosition);
		if ($this->isPost()) {
			...
			if ($m_user->create()) {
				if(isset($_POST['passwords']) && $_POST['passwords']!=''){
					$m_user->password = md5(md5(trim($_POST["passwords"])) . $user['salt']);
				} else {
