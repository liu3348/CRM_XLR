...
class UserController extends CommonController {
    ...
    public function index() {
        ...
        $vipnums=$this->user_vipnums($_SESSION['user_name']);
        $list1 = array(array('VIP'=>$vipnums));
        $arr_web=CallWebserviceVipAmount($list1);
        if($arr_web['status'] > 0)
        {
            // $web_sql='update'.$ecs->table('users') . ' set user_money="' .$arr_web['Amount'] .'" where vipnums= '.$vipnums;

            // $db->query($web_sql);
            $this->model->table('users')->where(array('vipnums'=>$vipnums))->data(array('user_money'=>$arr_web['Amount']))->update();
            file_put_contents("d:\\logs\\".date('Y-m-d')."sendusermoney_info.txt"," 卡号：".$vipnums."同步成功." . date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
        }
        else
        {
            file_put_contents("d:\\logs\\".date('Y-m-d')."sendusermoney_error.txt"," 卡号：".$vipnums."同步失败.失败信息：" . $arr_web['msg'].'     '.date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
        }



        $list2 = array(array('VIP'=>$vipnums));
        $arr_web=CallWebserviceVipIntegral($list2);

        if($arr_web['status'] > 0)
        {
            // $web_sql='update'.$ecs->table('users') . ' set pay_points="' .$arr_web['IntegralAmount'] .'" where vipnums= '.$vipnums;

            // $db->query($web_sql);
            $this->model->table('users')->where(array('vipnums'=>$vipnums))->data(array('pay_points'=>$arr_web['IntegralAmount']))->update();
            file_put_contents("d:\\logs\\".date('Y-m-d')."senduserIntegral_info.txt"," 卡号：".$vipnums."同步成功." . date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
        }
        else
        {
            file_put_contents("d:\\logs\\".date('Y-m-d')."senduserIntegral_error.txt"," 卡号：".$vipnums."同步失败.失败信息：" . $arr_web['msg'].'     '.date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
        }
    ...
    public function user_vipnums($username){
        $res= $this->model->table('users')->where(array('user_name'=>$username))->field('vipnums')->getOne();
        return $res;
    }
    ...
    public function profile() {
        ...
        if (IS_POST){
            ...
            $users_arr = $this->model->table('users')
                ->field('*')
                ->where(array('user_id'=>$user_id))
                ->select();
            // if($users_arr['0']['birthday']=='1956-01-01'||$users_arr['0']['birthday']=='0001-01-01'||$users_arr['0']['birthday']=='0000-00-00'){
            //  $birthday = trim($_POST['birthdayYear']) .'-'. trim($_POST['birthdayMonth']) .'-'.
            //   trim($_POST['birthdayDay']);
            //   }
            $birthday=substr(trim($_POST['Id_card']),6,4).'-'.substr(trim($_POST['Id_card']),10,2).'-'.substr(trim($_POST['Id_card']),12,2);


            $sex = substr($_POST['Id_card'], (strlen($_POST['Id_card'])==18 ? -2 : -1), 1) % 2 ? '1' : '2';
            ...
            $where_user['nc']=$nc;
            $where_user['Id_card']=$Id_card;
            if($birthday){
                $where_user['birthday']=$birthday;
            }
            if($sex){
                $where_user['sex']=$sex;
            }
            ...
            if (model('Users')->edit_profile($profile)) {
                ...
                $a=model('Users')->get_profile($user_id);
                ...
                $brithday=$a[birthday];
                $begaindate=date('Y-m-d',$a[reg_time]);
                $Id_card=$a['Id_card'];


                $list = array('MobileTel'=>$username,'Name'=>$nc,'Sex'=>$sex,'BrithDate'=>$brithday,'BegainDate'=>$begaindate,'VipGrade'=>'会员卡','IDCard'=>$Id_card);
                $arr_web=CallWebserviceAddvip($list);

                //return array('status'=>1,'MobileTel'=>$result_arr['AddVipResult']['VipResponseList']['VipNo']['MobileTel'],'VIP'=>$result_arr['AddVipResult']['VipResponseList']['VipNo']['VIP']);
                if($arr_web['status'] > 0)
                {
                    // 建立卡号和用户的关联 vipnums
                    // $web_sql='update'.$ecs->table('users') . ' set vipnums="' .$arr_web['VIP'] .'" where user_name= '.$username;
                    // $db->query($web_sql);
                    $this->model->table('users')->where(array('user_name'=>$username))->data(array('vipnums'=>$arr_web['VIP']))->update();
                    file_put_contents("d:\\logs\\".date('Y-m-d')."edit_info.txt"," 用户：".$username."修改成功.        " . date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
                }
                else
                {
                    file_put_contents("d:\\logs\\".date('Y-m-d')."edit_info.txt"," 用户：".$username."同步失败.失败信息：      " . $arr_web['msg'].'     '.date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
                }
    ...
    public function return_request() {
        if($_POST["type"]=="do"){
            $refund_status_s = mysql_fetch_array(mysql_query("SELECT refund_status FROM e_order_goods WHERE rec_id='".$_POST["rec_id"]."'"));
            $refund_status=$refund_status_s['refund_status'];

            if($refund_status!=1){
                $goods_sn_sql = mysql_fetch_assoc(mysql_query("SELECT goods_sn FROM e_order_goods WHERE rec_id='".$_POST["rec_id"]."'"));
                $goods_sn=$goods_sn_sql[goods_sn];
                // var_dump($goods_sn);
                // die();
                $integral_sql = mysql_fetch_assoc(mysql_query("SELECT give_integral FROM e_goods WHERE goods_sn='".$goods_sn."'"));
                $integral=$integral_sql[give_integral];
                if($integral>0)
                {
                    $user_id=$_SESSION['user_id'];
                    $pay_points_sql = mysql_fetch_assoc(mysql_query("SELECT pay_points FROM e_users WHERE user_id='".$user_id."'"));
                    $pay_points=$pay_points_sql[pay_points];
                    // var_dump($pay_points);
                    // var_dump($integral);
                    // die();
                    if($pay_points<$integral)
                    {
                        show_message("积分不够，暂不能退货","返回","/mobile/index.php?m=default&c=user&a=order_list");
                    }
                    else
                    {
                        $dir="mobile/data/refund_img/".md5($_POST['rec_id']);
                        //var_dump($dir);
                        // die();
                        if(is_dir($dir))
                        {
                            $res=model('Users')->upload_multi_file($_FILES,$dir);
                            // var_dump($res);
                            // die();
                            $imgs="refund_pic1='".$res[0]."',refund_pic2='".$res[1]."',refund_pic3='".$res[2]."'";
                            // var_dump($imgs);
                            // die();
                            $sql="update e_order_goods set refund_status=1,refund_reason='".$_POST["refund_reason"]."',refund_add_time='".time()."',$imgs where rec_id=".$_POST["rec_id"];
                            // var_dump($sql);
                            // die();
                            if(MYSQL_query($sql)){
                                show_message("退货申请提交成功","返回","/mobile/index.php?m=default&c=user&a=order_list");
                            }
                        }
                        else
                        {
                            mkdir("mobile/data/refund_img/".md5($_POST['rec_id']),0700,true);
                        }
                    }
                ...
                $this->display('return_request.dwt');
    ...
    public function refund_request() {
        if($_POST["type"]=="do"){
            ...
            $refund_status=$refund_status_s['refund_status'];
            if($refund_status!=3){
                $money = mysql_fetch_array(mysql_query("SELECT goods_number FROM e_order_goods WHERE rec_id='".$_POST["rec_id"]."'"));
                $goods_number=$money['goods_number'];
                // var_dump($goods_number);
                // die();
                if($_POST["refund_number"] <= $goods_number){
                    $sql="update e_order_goods set refund_status=3,refund_ems='".$_POST["refund_ems"]."',refund_add_time='".time()."',refund_company='".$_POST["refund_company"]."',refund_number='".$_POST["refund_number"]."' where rec_id=".$_POST["rec_id"];
                    // var_dump($sql);
                    // die();
                }
                else{
                    show_message("货品数量大于购买数量，请重新填写","返回","/mobile/index.php?m=default&c=user&a=order_list");
                }

                $xx = MYSQL_query($sql);
                if($xx){
                    show_message("退款申请提交成功","返回","/mobile/index.php?m=default&c=user&a=order_list");
                }
            }
            else{
                $this->display('refund_request.dwt');
            }
        }
    ...
    public function affirm_received() {
        ...
        if (model('Users')->affirm_received($order_id, $this->user_id)) {
            ...
            //折返码生成记录
            $code = mysql_fetch_row(mysql_query("select zf_code,order_sn from e_order_info WHERE order_id='".$order_id."'"));
            //var_dump($code);
            $code_m=$code['0'];
            $code_order=$code['1'];
            if($code_m !=''){
                //var_dump($code_m);
                $rebate = mysql_fetch_row(mysql_query("select rebate,uid from e_code WHERE code='".$code_m."'"));
                // var_dump($rebate);
                $rebate_m=$rebate['0'];
                // var_dump($rebate_m);
                $uid=$rebate['1'];
                model('ClipsBase')->log_account_change($uid, $rebate_m, 0, 0, 0, "获得".$code_order."折扣码返利",ACT_CODE);
            }
            // var_dump($rebate);
            // die();
    ...
    public function edit_surplus() {
        ...
        model('ClipsBase')->log_account_change($user['user_id'], (- 1) * $surplus, 0, 0, 0, $change_desc,ACT_OTHER);
        // 销量
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            if (0 == $enabled_sms) {
                ...
                $email = isset($_POST['email']) ? in($_POST['email']) : $_POST['username'].'@'.'hbjzx'.'.com'.'.cn';
                $password = isset($_POST['password']) ? in($_POST['password']) : '';
                $Id_card=isset($_POST['Id_card']) ? in($_POST['Id_card']) : '';
                $birthday=substr(trim($_POST['Id_card']),6,4).'-'.substr(trim($_POST['Id_card']),10,2).'-'.substr(trim($_POST['Id_card']),12,2);
                $other = array();
                ...
                if($_POST['type']==1){
                    ...
                    $_SESSION['register_smscode']=$smscode;
                    $_SESSION['register_phone']=$phone;
                    $sms_res = model('Users')->my_send_sms($phone,$smscode);
                }

                $smscode = isset($_POST['sms_code']) ? in($_POST['sms_code']) : '';
                if($smscode == '' || !is_numeric($smscode))
                {
                    show_message('请正确的输入短信验证码', 返回, 'index.php?m=default&c=user&a=register', 'error');
                }
                //检查短信验证码
                if(isset($_SESSION['register_smscode']) && !empty($_SESSION['register_smscode']))
                {
                    // var_dump(111111111);
                    if($_SESSION['register_smscode'] != $smscode)
                    {
                        show_message('短信验证码错误，请重新输入', 返回, 'index.php?m=default&c=user&a=register', 'error');
                    }
                }
            ...
            elseif (1 == $enabled_sms) {
                ...
                // 设置一个默认的邮箱
                $email = $username . '@hbjzx.com.cn';
            } else {
                ECTouch::err()->show(L('sign_up'), url('register'));
            }

            if (model('Users')->register($username, $password, $email,$Id_card,$birthday, $other) !== false) {
                /*把新注册用户的扩展信息插入数据库*/
                ...
                //推送会员信息并获取VIP卡号 'VipGrade'=>'1'
                $list = array('MobileTel'=>$username,'Name'=>$username,'Sex'=>'1','BrithDate'=>$birthday,'BegainDate'=>date('Y-m-d',time()),'VipGrade'=>'会员卡');
                $arr_web=CallWebserviceAddvip($list);

                // var_dump($arr_web['status']);
                // die();
                ...
                if($arr_web['status'] > 0)
                {
                    ...
                    // var_dump($vvv);
                    // die();
                    file_put_contents("d:\\logs\\".date('Y-m-d')."senduser_info.txt"," 用户：".$username."同步成功.        " . date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
                }
                else
                {
                    file_put_contents("d:\\logs\\".date('Y-m-d')."senduser_error.txt"," 用户：".$username."同步失败.失败信息：      " . $arr_web['msg'].'     '.date('Y-m-d H:i:s', time()) . "\n", FILE_APPEND);
                }
    ...
    public function ajaxsendsms(){
        // echo '<pre>';
        $user_name=$_POST['mobile'];
        $result = $this->model->table('users')->where(array('user_name'=>$user_name))->find();
        if($result){
            $smscode=rand(10000,99999);
            $_SESSION['modify_smscode']=$smscode;
            $_SESSION['modify_phone']=$user_name;
            $sms_res = model('Users')->my_send_sms($user_name,$smscode);
            $e=1;
            echo $e;
        }else{
            $e=0;
    ...
    public function modify_password(){
        ...
        $ec_salt= $this->model->table('users')->where(array('user_name'=>$user_name))->field('ec_salt')->getOne();
        $pwd=md5(md5($_POST['pwd']).$ec_salt);
        $pwd_result=$this->model->table('users')->where(array('user_name'=>$user_name))->field('password')->getOne();


        if($pwd==$pwd_result){
            show_message("系统检测到新密码与原密码一致，无法更改！", $_LANG['back_home_lnk'], url('get_password_phone'), 'info');

        }else{
            $smscode = isset($_POST['mobile_code']) ? trim($_POST['mobile_code']) : '';
            if($_SESSION['modify_smscode'] != $_POST['mobile_code'])
            {
                show_message('短信验证码错误，请重新输入.', 返回上一页, url('get_password_phone'), 'info');
            }else{
                $ec_salt = substr(uniqid(rand()), -6);
                $pwd=md5(md5($_POST['pwd']).$ec_salt);
                $sql=$this->model->table('users')->where(array('user_name'=>$user_name))->data(array('password'=>$pwd,'ec_salt'=>$ec_salt))->update();
                if ($sql) {
                    show_message('修改密码成功.', 返回主页, './', 'info');
                }
            }
        }
    ...
    public function send_pwd_email() {
        ...
        if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2))) {
            ...
            // 检查验证码
            if ($_SESSION['ectouch_verify'] !== strtoupper($_POST['captcha'])) {
                show_message(L('invalid_captcha'), L('back_page_up'), url('get_password_phone'), 'error');
            }
        }
