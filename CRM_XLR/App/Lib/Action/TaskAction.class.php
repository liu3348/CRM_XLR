...
//		订单类型数据引用  yb
		$objObj = M('OrderObj');
		$arrObj = $objObj->order('order_id desc')->select();
		$this->assign('arrObj',$arrObj);
		...
		if ($this->isPost()) {
			...
			if ($task = $m_task->create()) {
				...
				$task['order_id'] = isset($_POST['order_id']) ? $_POST['order_id'] : '1';
				$task['written_time'] = isset($_POST['issue_date']) ? strtotime($_POST['issue_date']) : '';
				$task['taking_time'] = isset($_POST['order_date']) ? strtotime($_POST['order_date']) : '';
...
//		var_dump($task_list[1]['about_roles']);
		foreach($task_list as $k=>$v){
			$ture = 1;
			foreach(explode(',',$v['about_roles']) as $kv=>$vv){
				if($vv!=''){
					$where = "module='task'and module_id=".$v['task_id'].' and creator_role_id='.$vv;
//					echo $where.'/////';
					if(!$objComment->where($where)->select()){
						$ture = 0;
						break;
						return false;
					}
...
//			session('role_id')
			...
			$objOrderObj = M('order_obj');
			$arrOrderObj = $objOrderObj->select();
...
//			var_dump($show_pl);die();
			...
			if($owner_role_id || $about_roles || $res || session('?admin')){
				...
				foreach($arrOrderObj as $k=>$v){
					if($v['order_id']==$task['order_id']){
						$task['order_id'] = $v['obj_name'];
					}
				}
				$this->task = $task;
				$this->alert = parseAlert();
