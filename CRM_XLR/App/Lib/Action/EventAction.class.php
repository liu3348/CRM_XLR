...
class EventAction extends Action{
		...
		if($this->isPost()){
			...
			if($m_event->create()){
				...
				if($event_id = $m_event->add()){
					$arrNew = array();
					if($_POST['get_url']){
						$arrNew['get_url'] = $_POST['get_url'];
					}
					if($_POST['presentation']){
						$arrNew['presentation'] = $_POST['presentation'];
					}
					if($arrNew[0]!='null'){
						$m_event->where('event_id='.$event_id)->save($arrNew);
					}
	...
	**/
	...
	public function index(){
		...
		//customer&a=view  页面数据过滤  Yb
		if($_GET['from_url']=="customer"){
			foreach($list as $k=>$v){
				if($v['owner_role_id']==$_GET['owner_role_id']){
...
//			echo "<pre>";
//			var_dump($currentSheet);die();
//			echo "<pre>";
			for ($currentRow = 3;$currentRow <= $allRow;$currentRow++) {
				$data['customer_id'] = $currentSheet->getCell('A'.$currentRow)->getValue();
//				$data['公司名称'] = $currentSheet->getCell('B'.$currentRow)->getValue();
//				$data['公司网址'] = $currentSheet->getCell('C'.$currentRow)->getValue();
				$data['subject'] = $currentSheet->getCell('D'.$currentRow)->getValue();
				$data['get_url'] = $currentSheet->getCell('E'.$currentRow)->getValue();
				$data['presentation'] = $currentSheet->getCell('F'.$currentRow)->getValue();
				$data['description'] = $currentSheet->getCell('G'.$currentRow)->getValue();
				//客户id
				$data['creator_role_id'] = $_SESSION['user_id'];
				//客户部门
				$objRole = M("Role");
				$arrRole = $objRole->where('role_id='.$_SESSION['role_id'])->select();
				$objPosition = M('Position');
				$arrPosition = $objPosition->where('position_id='.$arrRole[0]['position_id'])->select();
				$data['venue'] = $arrPosition[0]['name'];
				$data['owner_role_id'] = $_SESSION['role_id'];
				//开始时间和结束时间都是导入时间
...
//				$data['update_date'] = strtotime($currentSheet->getCell('Q'.$currentRow)->getValue());
				$event_id = $m_event->add($data);
//				echo "<pre>";
//				var_dump($data);
//				print_r($m_event->getLastSql());die();
				if(!$event_id) {
					...
					$objRCustomerEvent = M('RCustomerEvent');
					$arrRcustomerEvent['customer_id'] = $data['customer_id'];
					$arrRcustomerEvent['event_id'] = $event_id;
					$objRCustomerEvent->add($arrRcustomerEvent);
				}
			}
//			echo "<pre>";
//			print_r($data);die();
			if($_POST['owner_name']){
				$objUser = M('User');
				$arrUser = $objUser->where('name="'.$_POST['owner_name'].'"')->select();
				$to_role = $arrUser[0]['user_id'];
				if(is_array($to_role)){
					foreach($to_role as $k=>$v){
						sendMessage($v, $_SESSION['name']."于".date('Y-m-d').' 导入 '.$message_Row_length.' 条工作日程');
					}
					alert('success',L('SEND_SUCCESS'),U('event/index'));
				}else{
					if(sendMessage($to_role, $_SESSION['name']."于".date('Y-m-d').' 导入 '.$message_Row_length.' 条工作日程')){
						alert('success',L('SEND_SUCCESS'), U('event/index'));
					}else{
						alert('error',L('SEND_FAILY'),U('event/index'));
					}
				}
			}
