	...
	if ($position_list) {
		...
		foreach($position_list AS $value) {
			$department_name = M('RoleDepartment')->where('department_id = %d', $value['department_id'])->getField('name');
					//带横线的不显示										↓↓(and status!=2)
			$user_list = D('RoleView')->where('position.position_id = %d and status!=2', $value['position_id'])->select();
			$user_str = '';
                ...
                case 'box' :
                    ...
                    if ($setting['type'] == 'select') {
                        ...
                        $str = '';
                                $i = '';
						$pd = 0;
                        foreach ($setting['data'] as $k=>$v2) {
							//默认选择第一个   start  Yb
							if($pd==0){
								$pd=1;
								$str .= " &nbsp; <input type='radio'checked='true' name='".$v['field']."' id='".$v['field'].$i."' value='$v2'";
							}else{
...
//百度查询
function  send_seo($wd,$userUrl,$rn=30,$max_length=11){
	//客户百度排行搜索
	//加载phpQuery
	import('Org.Amin.QueryList');
	//百度最大查询数量 $rn
	//查询字段 $wd
	$userUrl = $userUrl.'/';
	//客户地址 $userUrl
	...
	$pd = 0;
	$paiX = 0;
	$html = file_get_contents('http://www.baidu.com/s?wd='.$wd.'&rn='.$rn);
	//然后可以把页面源码或者HTML片段传给QueryList
	for($i=1;$i<$max_length;$i++){
		$data = QueryList::Query($html,array(
				'title' => array('#'.$i.' .t','text'),
				'url' => array('#'.$i.' .c-showurl','text'),
				'url1' => array('#'.$i.' div .c-showurl','text'),
				'href' => array('#'.$i.' .f13 a[target=_blank]','href'),
		));
		//打印结果
		//排序
		$data->jsonArr[0]['id'] = $i;
		//过滤处网站地址
		preg_match_all('/(.*)?\//',$data->jsonArr[0]['url'],$m);
		$data->jsonArr[0]['pregUrl']=$m[0][0];
		preg_match_all('/(.*)?\//',$data->jsonArr[0]['url1'],$n);
		$data->jsonArr[0]['pregUrl1']=$n[0][0];
		if($userUrl==$m[0][0]||$userUrl==$n[0][0]||strstr($m[0][0],$userUrl)||strstr($n[0][0],$userUrl)){
			$data->jsonArr[0]['select_time'] = date('Y-m-d H:i:s');
			if($pd==0){
			$paiX=$i;
			$pd=1;
			}
		}
		$arrData['arr'][] = $data->jsonArr[0];
	}
	$arrData['paixu'] = $paiX;
	return($arrData);
}
//修改数据库前缀  _yb  2016/08/27
/**
 * @dbserver : 服务器地址
 * @dbname	: 数据库名
 * @dbuser	: 用户名
 * @dbpassword	: 密码
 * @old_prefix	: 前缀('xx_')
 * @new_prefix	: 修改后前缀('yy_')
 **/
function updateMysql_prefix($dbserver,$dbname,$dbuser,$dbpassword,$old_prefix,$new_prefix){
	if (
