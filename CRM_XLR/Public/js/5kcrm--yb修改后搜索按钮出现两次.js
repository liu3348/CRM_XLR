function changeCondition(){
	...
	//var serach_html  = '<input id="search" type="text" class="input-medium search-query" name="search">';
	//var dosearch_html = '<input type="hidden" name="m" value="customer"/><button type="button" id="dosearch" class="btn"> <img src="/Public/img/search.png">  搜索</button>';
	if(a == 'number') {
		...
		if($("input[name='m']").val()=="customer"||$("input[name='m']").val()=="contacts"){
			$("#searchContent").html(serach_html+dosearch_html);
		}else {
		$("#searchContent").html(serach_html);
		}
	}
		...
		$("#conditionContent").html('<select id="condition" style="width:auto" name="condition" onchange="changeSearch()">'
				...
				+'</select>&nbsp;&nbsp;');
		$("#searchContent").html('<input id="search" type="text" class="input-medium search-query" name="search" onclick="WdatePicker()"/>&nbsp;&nbsp;'+dosearch_html);
	}
	else if (a == 'bool') {
		$("#conditionContent").html('<select id="condition" style="width:auto" name="condition" onchange="changeSearch()">'
		...
		$.ajax({
			...
			success:function(data){
				...
				if($("input[name='m']").val()=="customer"){
					$("#searchContent").html('<select id="search" style="width:auto" name="search">' + options + '</select>&nbsp;&nbsp;'+dosearch_html);
				}else {
					$("#searchContent").html(serach_html);
				}
				$("#conditionContent").html('');
		...
		$("#searchContent").html('<select name="state" id="state" style="width:auto"></select>'
				...
				+'<input type="text" id="search" name="search" placeholder='+CrmLang.STREET_INFORMATION+' class="input-large">&nbsp;&nbsp;'+dosearch_html);
		new PCAS("state","city","","");
	}
	else if(a=='order_id'){
		$.post('',{type:'order_type'},
				function(data){
					var htmlText = '';
					var objJson = eval(data);
					for (var i=0;i<objJson.length;i++)
					{
						htmlText += "<option value='"+objJson[i]['order_id']+"'>"+objJson[i]['obj_name']+"</option>";
						if(i==objJson.length*1-1){
							$("#conditionContent").html('<select id="condition" style="width:auto" name="search" onchange="changeSearch()">' +htmlText+
									"</select>");
							$("#searchContent").html('');
						}
